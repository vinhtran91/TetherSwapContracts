...
// File: contracts/interfaces/ITetherswapFactory.sol
...
// File: contracts/interfaces/ITetherswapERC20.sol
...
// File: contracts/interfaces/ITetherswapPair.sol
...
// File: contracts/interfaces/ITetherswapRouter.sol
...
// File: contracts/libraries/SafeMathTetherswap.sol
...
// File: contracts/libraries/TetherswapLibrary.sol
...
library TetherswapLibrary {
    ...
    function getReserves(
        ...
        (uint256 reserve0, uint256 reserve1, ) = ITetherswapPair(ITetherswapFactory(factory).getPair(tokenA, tokenB))
            .getReserves();
    ...
    function quote(
        ...
        require(reserveA > 0 && reserveB > 0, "TetherswapLibrary: INSUFFICIENT_LIQUIDITY");
    ...
    function getAmountOut(
        ...
        require(reserveIn > 0 && reserveOut > 0, "TetherswapLibrary: INSUFFICIENT_LIQUIDITY");
    ...
    function getAmountsOut(
        ...
        for (uint256 i; i < path.length - 1; i++) {
            ...
            amounts[i + 1] = getAmountOut(
                ...
                reserveIn,
                reserveOut,
                ITetherswapPair(ITetherswapFactory(factory).getPair(path[i], path[i + 1])).tradingFeePercent()
            );
        }
    }
    ...
    function getAmountsIn(
        ...
        for (uint256 i = path.length - 1; i > 0; i--) {
            ...
            amounts[i - 1] = getAmountIn(
                ...
                reserveIn,
                reserveOut,
                ITetherswapPair(ITetherswapFactory(factory).getPair(path[i - 1], path[i])).tradingFeePercent()
            );
        }
    }
}
...
// File: contracts/TetherswapRouter.sol
...
contract TetherswapRouter is ITetherswapRouter {
    ...
    function _addLiquidity(
        ...
        require(ITetherswapFactory(factory).getPair(tokenA, tokenB) != address(0), "TetherswapRouter: PAIR_NOT_CREATED");
        (uint256 reserveA, uint256 reserveB) = TetherswapLibrary.getReserves(factory, tokenA, tokenB);
        if (reserveA == 0 && reserveB == 0) {
            (amountA, amountB) = (amountADesired, amountBDesired);
        } else {
            uint256 amountBOptimal = TetherswapLibrary.quote(amountADesired, reserveA, reserveB);
            if (amountBOptimal <= amountBDesired) {
                require(amountBOptimal >= amountBMin, "TetherswapRouter: INSUFFICIENT_B_AMOUNT");
                (amountA, amountB) = (amountADesired, amountBOptimal);
            } else {
                uint256 amountAOptimal = TetherswapLibrary.quote(amountBDesired, reserveB, reserveA);
                assert(amountAOptimal <= amountADesired);
                require(amountAOptimal >= amountAMin, "TetherswapRouter: INSUFFICIENT_A_AMOUNT");
    ...
    function removeLiquidity(
        ...
        require(amountA >= amountAMin, "TetherswapRouter: INSUFFICIENT_A_AMOUNT");
        require(amountB >= amountBMin, "TetherswapRouter: INSUFFICIENT_B_AMOUNT");
    }
    ...
    function removeLiquidityWithPermit(
        ...
        ITetherswapPair(pair).permit(msg.sender, address(this), value, deadline, v, r, s);
    ...
    function _swap(
        ...
        for (uint256 i; i < path.length - 1; i++) {
            ...
            (uint256 amount0Out, uint256 amount1Out) = input == token0
                ? (uint256(0), amountOut)
                : (amountOut, uint256(0));
            address to = i < path.length - 2 ? TetherswapLibrary.pairFor(factory, output, path[i + 2]) : _to;
            ITetherswapPair(TetherswapLibrary.pairFor(factory, input, output)).swap(
                amount0Out,
                amount1Out,
                to,
                new bytes(0)
    ...
    function swapExactTokensForTokens(
        ...
        require(amounts[amounts.length - 1] >= amountOutMin, "TetherswapRouter: INSUFFICIENT_OUTPUT_AMOUNT");
    ...
    function swapTokensForExactTokens(
        ...
        require(amounts[0] <= amountInMax, "TetherswapRouter: EXCESSIVE_INPUT_AMOUNT");
        TransferHelper.safeTransferFrom(
            path[0],
            msg.sender,
            TetherswapLibrary.pairFor(factory, path[0], path[1]),
    ...
    function swapExactETHForTokens(
        ...
        assert(IWETH(WETH).transfer(TetherswapLibrary.pairFor(factory, path[0], path[1]), amounts[0]));
        _swap(amounts, path, to);
    }
    ...
    function swapTokensForExactETH(
        ...
        require(path[path.length - 1] == WETH, "TetherswapRouter: INVALID_PATH");
        amounts = TetherswapLibrary.getAmountsIn(factory, amountOut, path);
        require(amounts[0] <= amountInMax, "TetherswapRouter: EXCESSIVE_INPUT_AMOUNT");
        TransferHelper.safeTransferFrom(
            path[0],
    ...
    function swapETHForExactTokens(
        ...
        require(path[0] == WETH, "TetherswapRouter: INVALID_PATH");
        amounts = TetherswapLibrary.getAmountsIn(factory, amountOut, path);
        require(amounts[0] <= msg.value, "TetherswapRouter: EXCESSIVE_INPUT_AMOUNT");
        IWETH(WETH).deposit{value: amounts[0]}();
        assert(IWETH(WETH).transfer(TetherswapLibrary.pairFor(factory, path[0], path[1]), amounts[0]));
        _swap(amounts, path, to);
        // refund dust eth, if any
    ...
    function _swapSupportingFeeOnTransferTokens(address[] memory path, address _to) internal virtual {
        for (uint256 i; i < path.length - 1; i++) {
            (address input, address output) = (path[i], path[i + 1]);
            (address token0, ) = TetherswapLibrary.sortTokens(input, output);
            ITetherswapPair pair = ITetherswapPair(TetherswapLibrary.pairFor(factory, input, output));
            uint256 amountInput;
    ...
    function swapExactETHForTokensSupportingFeeOnTransferTokens(
        ...
        assert(IWETH(WETH).transfer(TetherswapLibrary.pairFor(factory, path[0], path[1]), amountIn));
        uint256 balanceBefore = IERC20(path[path.length - 1]).balanceOf(to);
        _swapSupportingFeeOnTransferTokens(path, to);
        require(
            IERC20(path[path.length - 1]).balanceOf(to).sub(balanceBefore) >= amountOutMin,
    ...
    function swapExactTokensForETHSupportingFeeOnTransferTokens(
        ...
        require(amountOut >= amountOutMin, "TetherswapRouter: INSUFFICIENT_OUTPUT_AMOUNT");
    ...
    function getAmountOut(
        ...
        return TetherswapLibrary.getAmountOut(amountIn, reserveIn, reserveOut, tradingFeePercent);
    }
    ...
    function getAmountIn(
        ...
        return TetherswapLibrary.getAmountIn(amountOut, reserveIn, reserveOut, tradingFeePercent);
    }
