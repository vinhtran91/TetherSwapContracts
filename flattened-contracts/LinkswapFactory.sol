...
// File: contracts/interfaces/ITetherswapPriceOracle.sol
...
// File: contracts/interfaces/ITetherswapCallee.sol
...
// File: contracts/TetherswapPair.sol
...
contract TetherswapPair is ITetherswapPair, ReentrancyGuard {
    ...
    modifier onlyGovernance() {
        require(msg.sender == ITetherswapFactory(factory).governance(), "Pair: FORBIDDEN");
        _;
    }
    ...
    function _mintFee(uint112 _reserve0, uint112 _reserve1) private returns (bool feeOn) {
        uint256 protocolFeeFractionInverse = ITetherswapFactory(factory).protocolFeeFractionInverse();
        ...
        if (feeOn) {
            if (_kLast != 0) {
                ...
                if (rootK > rootKLast) {
                    ...
                    if (liquidity > 0) {
                        ITetherswapFactory TetherswapFactory = ITetherswapFactory(factory);
                        uint256 treasuryProtocolFeeShare = TetherswapFactory.treasuryProtocolFeeShare();
                        _mint(TetherswapFactory.treasury(), liquidity.mul(treasuryProtocolFeeShare) / 1000000);
                        _mint(
    ...
    function swap(
        ...
        uint256 balance1;
        {
            ...
            if (data.length > 0) ITetherswapCallee(to).TetherswapCall(msg.sender, amount0Out, amount1Out, data);
            balance0 = IERC20(_token0).balanceOf(address(this));
            balance1 = IERC20(_token1).balanceOf(address(this));
            if (ITetherswapFactory(factory).maxSlippagePercent() > 0) {
                ...
                if (priceAtLastSlippageBlocks == 0) {
                    ...
                    bool resetSlippage = lastSlippageBlocks.add(ITetherswapFactory(factory).maxSlippageBlocks()) <
                        block.number;
                    uint256 lastPrice = resetSlippage ? lastSwapPrice : priceAtLastSlippageBlocks;
                    require(
                        currentPrice >=
                            lastPrice.mul(uint256(100).sub(ITetherswapFactory(factory).maxSlippagePercent())) / 100 &&
                            currentPrice <=
                            lastPrice.mul(uint256(100).add(ITetherswapFactory(factory).maxSlippagePercent())) / 100,
                        "Pair: SlipLock"
                    );
                    if (resetSlippage) {
                        priceAtLastSlippageBlocks = currentPrice;
...
// File: contracts/TetherswapFactory.sol
...
contract TetherswapFactory is ITetherswapFactory, ReentrancyGuard {
    ...
    function _payListingFee(
        ...
        if (listingFeeToken == LINK) {
            listingFeeTokenAmount = ITetherswapPriceOracle(priceOracle).calculateTokenAmountFromUsdAmount(
                LINK,
                linkListingFeeInUsd
            );
    ...
    function createPair(
        ...
        if (!approvedPair[token0][token1]) {
            ...
            uint256 lockupAmountInUsd = ITetherswapPriceOracle(priceOracle).calculateUsdAmountFromTokenAmount(
                lockupToken,
                lockupTokenAmount
            );
        ...
        if (!approvedPair[token0][token1] && lockupTokenAmount > 0 && lockupPeriod > 0) {
            TetherswapPair(pair).listingLock(msg.sender, lockupPeriod, liquidity);
        }
    }
